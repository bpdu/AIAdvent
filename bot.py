...
def call_yandex_gpt(messages, current_date=None) -> str:
    ...
    system_prompt = """
Ты — помощник по генерации названий для стартапов. 
Придумай короткое, цепляющее название для приложения.

Придумай 5 неожиданных, ярких, запоминающихся названий. Используй метафоры, игры слов, юмор, даже абсурд.  
Чем нестандартнее — тем лучше. Добавь краткое пояснение к каждому.
    """
