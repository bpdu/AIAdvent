...
def call_yandex_gpt(prompt: str) -> str:
    ...
    system_prompt = (
        """
        Упакуй ответ в валидный JSON формат в формате: {"request": "мой запрос", "response": "твой ответ"}.
        Ответь на запрос простым, понятным текстом на русском языке. 
        Не используй обратные кавычки (`), не оформляй ничего как код, не используй Markdown или структурированные блоки. 
        ...
        {
        "request": "Сколько лет Сергею Жукову из Руки вверх?", 
        "response": "К сожалению, у меня нет актуальной информации о возрасте Сергея Жукова."
        }
        """
    )
